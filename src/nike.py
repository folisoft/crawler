...
async def craw(_nike_url):
    ...
    # GENDER
    gender_query = "Array.from(document.querySelectorAll('" + \
        _nike_gender_class+"')).map(item => item.textContent);"
    genders = await page.evaluate(gender_query, force_expr=True)

    data_crawler['Gender'] = 'Men' if 'Men' in genders[0] else 'Women'

    # SIZES
    query = "Array.from(document.querySelectorAll('"+_nike_sizes_class + \
        "')).map(item => ({disabled: item.disabled, size: item.value.split(':')[1]}));"
